...
def scaffold_with_all_fragments(
...
) -> list[tuple[str, str, Chem.Mol, str, int]]:
    """For every fragment in *frag_lib*, try attaching it at every linker site on *scaffold*.

    Returns a list of ``(name, frag_smiles, product_mol, smiles, site)`` tuples.
    ``name`` = fragment library name, ``frag_smiles`` = canonical fragment SMILES,
    ``site`` = scaffold atom index used for attachment.
    ...
    seen: set[str] = set()
    results: list[tuple[str, str, Chem.Mol, str, int]] = []
    ...
    for name, frag in frag_lib.fragments.items():
        ...
        for site in sites:
            ...
            if smiles in seen:
                continue
            seen.add(smiles)
            results.append((name, frag_smiles, product, smiles, site))
...
if __name__ == "__main__":
    ...
    for name, frag_smiles, product_mol, smiles, site in results:
        print(json.dumps({
            "SMILES": smiles,
